...
class Markov {
  ...
  addSource(source, regenerate = true) {
    ...
    source.forEach((i) => {
      ...
      ids.forEach((id) => {
        if (!gramIds.includes(id)) gramIds.push(id);
      });
    });
  ...
  addSequence(item, regenerate) {
    ...
    for (let order = 1; order <= maxOrder; order += 1) {
      for (let pos = 0; pos < sequence.length; pos += 1) {
        ...
        // Add to starting list if it's the first position.
        // if (pos === 1 && !this.starts.includes(gramId)) this.starts.push(gramId);
  ...
  generate({
    ...
    let currentOrder = start.length;
    for (let i = 0; i < max; i += 1) {
      const mask = i < min ? [this.end] : [];
      let gramId;
      
      for (let o = currentOrder; o > 0; o -= 1) {
        const gram = sequence.slice(sequence.length - currentOrder, sequence.length);
      ...
      if (currentOrder < order) currentOrder += 1;
      if (currentOrder === order) currentOrder -= 1;
    }
